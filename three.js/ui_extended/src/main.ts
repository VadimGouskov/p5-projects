...
const wobblyMix = new RangeControl("MIX", "general-controls", {
  min: 0,
  max: 1,
  step: 0.01,
  value: 0.5,
...
const sliceY = new RangeControl("SLICE_Y", "wobbly-controls", {
  min: 0,
  max: 10,
  step: 1,
  value: 0,
...
const getPerlinY = (col: number, row: number) => {
  const x = getRowSlice(row * perlinColSize.value) / COLS;
  const y = getColSlice(col * perlinRowSize.value) / ROWS;
